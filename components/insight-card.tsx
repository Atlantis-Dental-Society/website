...
export function InsightCard({ post, bannerUrl }: InsightCardProps) {
  return (
    <Link href={`/insights/${post.slug}`} className="group">
      <Card className="rounded-3xl border-none ring-0 overflow-hidden pt-0 shadow-warm hover:shadow-warm-lg transition-all hover:-translate-y-1">
        <div className={`aspect-[2/1] relative overflow-hidden ${!bannerUrl ? `bg-gradient-to-br ${categoryGradients[post.category ?? ""] || "from-primary/15 to-sage/10"} flex items-center justify-center` : ""}`}>
          {bannerUrl ? (
            <Image src={bannerUrl} alt={post.title ?? ""} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width: 640px) 100vw, 50vw" />
          ) : (
            <>
          ...
          <div className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full bg-warm-cream/20 opacity-0 group-hover:opacity-100 transition-opacity">
